...
UX-фича поверх существующих кирпичей (`planner.json` + `job-model` + `classifier`): Клод периодически спрашивает «что делал», предлагает 3 предсказания в виде инлайн-кнопок, помнит паттерны по дням недели, напоминает об обязательных делах и проверяет «сделал ли». Не отдельная подсистема — надстройка-сценарий.
...
2. **Периодический опрос «что делал?»**
   ...
   4. Если в окне ±30 мин есть planner-item — общий опрос подавляется (anti-spam).
3. **Дневной план (разовый)**
   1. Юзер кладёт items в `planner.json` на конкретную дату (`repeat: null`). Пример: 09:00 работа, 12:00 врач, 15:00 спорт, 20:00 домой, 22:00 спать.
...
5. **Память по дням недели**
   ...
   2. Где именно хранить — открытый вопрос ([Q-A-09](../questions/Q-A-09-tracker-memory-model.md)).
