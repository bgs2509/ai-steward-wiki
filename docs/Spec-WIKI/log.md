...
## [2026-05-10] refactor | tech-spec middle findings closed structurally

**Trigger:** review pass выявил 5 middle-уровня gap'ов (1 уже закрыт в коммите 1ae9dd4 — staging retention).

**Изменения в tech-spec-draft.md:**
1. §2 — новая подсекция «Cross-store identity mapping invariant»: mapping `user_id ↔ telegram_id` живёт только в `sessions.db.users`; FK на `user_id` из `jobs.db`/`audit.db` запрещены (ломают SIGHUP-sync D-031).
2. §3 — DLQ-таблица явно названа `jobs.db.jobs_dlq` с FK на `jobs.id`. Auto-disable: явно зафиксировано, что timeout (`killed`) считается failure-strike любой категории.
3. §5 — новая подсекция «Anti-spam (D-041 SSoT)»: hard cap 20, Levenshtein ≤2 с graduated confirm (D-023), `_trash/` exclusion. Закрывает Middle #6.
4. §6 — раскладка `prompts/` зафиксирована как `<service>/prompts/` с привязкой к §10.1 `ReadOnlyPaths`. Closed list файлов и domain-types.
5. §0 — добавлены INV-10 (cross-store FK convention), INV-11 (DLQ location), INV-12 (timeout = failure-strike), INV-13 (prompts path absolute), INV-14 (anti-spam SSoT в §5). Verification ritual расширен с 7 до 12 шагов.

**Цель:** сделать каждый middle gap structurally невозможным к регрессии — INV-10…INV-14 ловят их на pre-commit checklist.

**Cross-refs:** D-019, D-021, D-023, D-031, D-041, D-015, D-008; commits `1ae9dd4`, `761fa61`.

## [2026-05-12] decision | D-024 delivery-часть перенесена в ADR-024-digest-presentation

`docs/adr/ADR-024-digest-presentation.md` создан (bd `aisw-w3k`, Inbox-WIKI Phase-D.b.2a). D-024 §«Перенос в ADR»: delivery-часть (HTML parse_mode, TL;DR-секция, `<b>`-section split + `(n/m)`, `send_document` fallthrough, `data/runs/` persist, empty-line) — отмечена `[x]`. Interactive-часть (actionable ±2h cards, `/expand`, per-user toggles) — за bd `aisw-269` (Phase-D.b.2b), будущий ADR.

**Cross-refs:** D-024, D-025; ADR-007; commits в master ветке `ai-steward-wiki`.
