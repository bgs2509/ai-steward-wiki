...
>
> **Update ([D-030](../decisions/D-030-onboarding.md), 2026-05-10):** onboarding-вопросы берутся из локального template-файла repo сервиса, не из parent `ai-steward/CLAUDE.md`.
...
- [x] **Вариант D** — hybrid `users.toml` SSoT + готовый `/start`-flow за config-флагом:
  ...
  - **Multi-tenant phase** (флаг `ENABLE_SELF_SIGNUP=true`):
    1. `/start` → запись в `sessions.db.pending_users(telegram_id PK, telegram_username, ts, requested_lang, requested_timezone, requested_persona)`.
  ...
  - **Onboarding Q&A (всегда, после approve):**
    1. Claude-driven через Inbox-WIKI router; questions per local `templates/onboarding-profile-questions.<lang>.md`, aligned with D-042 user fields.
