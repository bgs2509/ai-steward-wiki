...
**Нормализация имени** — pipeline свободного имени из NL-промпта (`medical`, `Multi Word`, `multi-word`, `здоровье`):

1. Транслитерация `Cyrillic → Latin` (single-strategy ISO 9).
2. Split по non-alphanumeric.
3. PascalCase join (`multi word` → `MultiWord`).
4. Suffix `-WIKI` (`MultiWord-WIKI`).
5. Валидация regex `^[A-Z][A-Za-z0-9]*-WIKI$` (D-008).

**Lookup пресета** (§11) — case-insensitive:

- По slug `multiword`, **ИЛИ**
- По `multi-word`-варианту с восстановленным дефисом для имени файла шаблона.
- Если `templates/multiword.md` отсутствует и `templates/multi-word.md` существует — используется он.
- Иначе → `_default.md`.
...
- `<service>/prompts/domain-<type>.md` — добавляется к Stage-1b (`<type>` ∈ {medical, investment, budget, family, study, career, cooking, _default}).
...
- `medical`, `investment`, `budget`, `family`, `study`, `career`, `cooking`.
...
- Template lookup поддерживает primary slug (`multiword`) и hyphenated alias (`multi-word`) без расширения D-008 regex для WIKI-директорий.
